...
const CoursesAcceptReserve = () => {
  // ** Store Vars

  // ** States

  const [dataCourseReserve, setDataCourseReserve] = useState("");
  const [totalReseveCount, setTotalReseveCount] = useState("");
  ...
  // get courses list Reserved

  const getCourseReserve = async () => {
    const result = await http.get(`/CourseReserve`);
    console.log("result", result);
    setTotalReseveCount(result.totalCount);
    return result;
  };

  const { data, status, refetch } = useQuery(
    ["getCourseReserve", searchTerm, rowsPerPage,PageNumber],
    getCourseReserve
  );
  ...
  console.log("getCourseReserve", getCourseReserve);
  ...
  const CustomPagination = () => {
    const count = Number(Math.ceil(dataCourseReserve / rowsPerPage));
    return (
      <ReactPaginate
        previousLabel={""}
        nextLabel={""}
...
export default CoursesAcceptReserve;
