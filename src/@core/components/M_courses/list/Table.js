...
const CoursesList = () => {
  ...
  const getAllCourses = async () => {
    const result = await http.get(
      `/Course/CourseList?PageNumber=${PageNumber}&RowsOfPage=${rowsPerPage}&SortingCol=DESC&SortType=Expire&Query=${searchTerm}`
    );
    console.log(result);
    setTotalCourseCount(result.totalCount);
    console.log("ffffffffffffff",totalCourseCount)
    return result;
  };
  const { data, status, refetch } = useQuery(
    ["getAllCourses", searchTerm, rowsPerPage,PageNumber],
    getAllCourses
  );
  ...
  const CustomPagination = () => {
    const count = Number(Math.ceil(totalCourseCount / rowsPerPage));
    return (
      <ReactPaginate
        previousLabel={""}
        nextLabel={""}
