...
const UserStusent = () => {
  ...
  const getStudents = async () => {
    const result = await http.get(
      `/User/UserMannage?PageNumber=${PageNumber}&RowsOfPage=${rowsPerPage}&SortingCol=DESC&SortType=InsertDate&Query=${searchTerm}&IsActiveUser=true&IsDeletedUser=false&roleId=3`
    );
    console.log(result);
  ...
  const { data, status, refetch } = useQuery(
    ["getStudents", searchTerm, rowsPerPage,PageNumber],
    getStudents
  );
