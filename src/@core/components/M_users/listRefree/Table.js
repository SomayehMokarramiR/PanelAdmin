...
const UserRefree = () => {
  ...
  const getAllUsers = async () => {
    const result = await http.get(
      `/User/UserMannage?PageNumber=${PageNumber}&RowsOfPage=${rowsPerPage}&SortingCol=DESC&SortType=InsertDate&Query=${searchTerm}&IsActiveUser=true&IsDeletedUser=false&roleId=7`
    );
    console.log(result);
  ...
  const { data, status, refetch } = useQuery(
    ["getAllUsers", searchTerm, rowsPerPage,PageNumber],
    getAllUsers
  );
