...
const UserMentor = () => {
  ...
  const getMentors = async () => {
    const result = await http.get(
      `/User/UserMannage?PageNumber=${PageNumber}&RowsOfPage=${rowsPerPage}&SortingCol=DESC&SortType=InsertDate&Query=${searchTerm}&IsActiveUser=true&IsDeletedUser=false&roleId=9`
    );
    console.log(result);
  ...
  const { data, status, refetch } = useQuery(
    ["getMentors", searchTerm, rowsPerPage,PageNumber],
    getMentors
  );
  data &&console.log("mmmmmmmmmmmmmmmmm",data.listUser);


  useEffect(() => {
    if (status === "success") {
      setDataMentor(data.listUser);
      refetch();
    }
  }, [status, data]);
