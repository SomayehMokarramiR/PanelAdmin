...
const UserTeacher = () => {
  ...
  const getTeachers = async () => {
    const result = await http.get(
      `/User/UserMannage?PageNumber=${PageNumber}&RowsOfPage=${rowsPerPage}&SortingCol=DESC&SortType=InsertDate&Query=${searchTerm}&IsActiveUser=true&IsDeletedUser=false&roleId=2`
    );
    console.log(result);
  ...
  const { data, status, refetch } = useQuery(
    ["getTeachers", searchTerm, rowsPerPage,PageNumber],
    getTeachers
  );
