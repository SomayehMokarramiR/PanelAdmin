...
const ProReverseUserCourses = () => {
  // ** Store Vars

  // ** States
  const [dataReseveCourse, setDataReseveCourse] = useState();
  ...
  const getUserReseveCourses = async () => {
    const result = await http.get(`/User/UserDetails/${paramId.id}`);
    // setTotalUserCount(res.totalCount);
    // console.log("ffffffffffffff", totalUserCount);
    return result;
  };
  const { data, status, refetch } = useQuery(
    ["getUserReseveCourses", searchTerm, rowsPerPage, PageNumber],
    getUserReseveCourses
  );
  ...
  useEffect(() => {
    if (status === "success") {
      setDataReseveCourse(data.coursesReseves);
      refetch();
    }
  }, [status, data]);
    ...
    <Fragment>
      <Card className="overflow-hidden">
        <CardHeader className="text-center d-flex justify-content-center fw-bolder text-primary">
          <CardTitle tag="h4"> دوره های رزرو شده کاربر</CardTitle>
        </CardHeader>
        {dataReseveCourse?.length == 0 || totalUserCount == 0 ? (
          <div
            style={{
              background: "#fff",
              color: "#219143",
...
export default ProReverseUserCourses;
